...
"""
    ...
    for file in files:
        ...
        with open(file, 'r') as f:
            ...
            df['end_time'] = datetime.datetime.fromisoformat(data['end_time'])
            df['start_time'] = datetime.datetime.fromisoformat(data['start_time'])
            df['request_per_second'] = len(data['data']) / (df['end_time'] - df['start_time']).dt.total_seconds()
...
def calculate_statistics(df):
    ...
    metrics = [ 'connect_duration', 'request_sent_duration', 
               'response_first_byte_duration', 'full_duration','request_per_second']
    
    for metric in metrics:
        stats_dict[metric] = {
            "unit": "nanoseconds" if metric != 'request_per_second' else "requests/second",
            ...
            "quantiles": df[metric].quantile([0.25, 0.5, 0.75, 0.95, 0.99]).to_dict(),
